...
class ShapeNet55_fs(Dataset):
    def __init__(self,root,split='test',fold=2,point_support=False,data_aug=True,prj_num=14):
        super().__init__()
        self.root=root
        self.split=split
        self.fold=fold
    ...
    def get_query_img_data(self):
        ...
        proj_path=os.path.join(self.root,'Projection_mesh_darkbg')

        for cls_ind, i in enumerate(self.class_list):
            class_path=os.path.join(img_path,i)
            sample_fold_path=[os.path.join(class_path,j,'image_output') for j in sorted(os.listdir(class_path))]
            # # ==== Except the prj_num samples, rest are used for query set ====
            prj_num=len(os.listdir(os.path.join(proj_path,i)))
            sample_fold_path=sample_fold_path[prj_num:]
            # ===================================================
    ...
    def get_support_data(self):
        ...
        for cls_ind,i in enumerate(self.class_list):
            ...
            sample_list=sorted(os.listdir(class_path))
        ...
        for _ in range(self.episode):
            ...
            for picked_cls in picked_cls_set:
                ...
                if self.mode=='random':
                    ...
                    # ==== pick up from a certain view ====
                    # if self.dataset.split!='train':
                    #     bottom_list=[0,3,6,9,13]
                    #     center_list=[1,4,7,10]
                    #     up_list=[2,5,8,11,12]

                    #     target_view=up_list
                    #     legal_path=[i for i in legal_path if int(os.path.split(i)[1].split('.')[0]) in target_view]
                    # =====================================
...
def get_sets(data_path,fold=0,n_way=5,k_shot=1,query_num=10,point_support=True,prj_num=6,mode='random'):
    ...
    train_dataset=ShapeNet55_fs(root=data_path,split='train',fold=fold,point_support=point_support,prj_num=prj_num)
    ...
    test_dataset=ShapeNet55_fs(root=data_path,split='test',fold=fold,point_support=point_support,prj_num=prj_num)
...
if __name__=='__main__':
    data_root='the path of ShapeNet55-LS folder'
    point_support=True
    ...
    dataset=ShapeNet55_fs(root=data_root,split='test',fold=2,point_support=point_support,prj_num=14)
    dataset[0]
    # query_set=dataset.query_img_base
    # bar=tqdm(range(len(query_set)))
    # for i in bar:
    #     data_path=query_set[i]
    #     data=cv2.imread(data_path)
    #     trans(data)
    #     bar.update()
    ...
    samp=NShotTaskSampler(dataset,300,n_way=5,k_shot=1,query_num=5,point_support=point_support,mode='random')
