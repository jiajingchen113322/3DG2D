...
class ModelNet40_fs(Dataset):
    def __init__(self,root,split='train',fold=0,point_support=False,data_aug=True,prj_num=14):
        ...
        if self.split=='train':
            mask=1-mask
            mask[self.fold*10:(self.fold+1)*10]=0
            mask=mask.astype(np.bool)
            self.class_list=class_name_list[class_id[mask]]
        
        else:
            mask[self.fold*10:(self.fold+1)*10]=1
            mask=mask.astype(np.bool)
            self.class_list=class_name_list[class_id[mask]]
        # ================================
    ...
    def get_query_img_data(self):
        ...
        for cls_ind, i in enumerate(self.class_list):
            ...
            sample_fold_path=[os.path.join(class_path,j,'image_output') for j in sorted(os.listdir(class_path),key=lambda x:int(x[:-4].split('_')[-1]))]

            # # ==== Except the 50 samples, rest are used for query set ====
            sample_fold_path=sample_fold_path[50:]
            # ===================================================
    ...
    def get_support_data(self):
        ...
        for cls_ind,i in enumerate(self.class_list):
            ...
            sample_list=sorted(os.listdir(class_path),key=lambda x: int(x.split('_')[-1]))[:50]
...
class NShotTaskSampler(Sampler):
    def __init__(self, dataset, episode,n_way,k_shot,query_num,point_support=False,mode='one_obj'):
        ...
        for _ in range(self.episode):
            ...
            for picked_cls in picked_cls_set:
                ...
                if self.mode=='random':
                    ...
                    #     legal_view=[2,5,8,11] #up
                    #     legal_view=[0,3,6,9] #down
                    #     legal_view=[1,4,7,10] #horizon
                    #     legal_path=[i for i in legal_path if int(os.path.split(i)[1].split('.')[0]) in legal_view]
...
def get_sets(data_path,fold=0,n_way=5,k_shot=1,query_num=10,point_support=True,prj_num=14,mode='random'):
    if point_support:
        cfn=collect_fn
    else:
        cfn=collect_fn_onlyimg
    
    train_dataset=ModelNet40_fs(root=data_path,split='train',fold=fold,point_support=point_support,prj_num=prj_num)
    ...
    test_dataset=ModelNet40_fs(root=data_path,split='test',fold=fold,point_support=point_support,prj_num=prj_num)
...
if __name__=='__main__':
    data_root='The path of ModelNet40-LS folder'
    point_support=False
    ...
    # dataset=ModelNet40_fs(root=data_root,point_support=point_support,prj_num=14)
    # # dataset[len(dataset)-300]
