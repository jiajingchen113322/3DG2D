...
class PCViews:
    ...
    def __init__(self):
        ...
        #     [[1 * np.pi / 2, 0, np.pi / 2+np.pi/4], [0, 0, TRANS]],
        #     [[2 * np.pi / 2, np.pi / 4, np.pi / 2], [0, 0, TRANS]],
        #     [[3 * np.pi / 2, 0, np.pi / 2-np.pi/4], [0, 0, TRANS]],
        #     [[0 * np.pi / 2, -np.pi / 4,  np.pi / 2], [0, 0, TRANS]],
            
            # [[0, -np.pi / 4, np.pi / 2], [0, 0, TRANS]],
            # [[1 * np.pi / 4,-np.pi / 4, np.pi / 2], [0, 0, TRANS]],
            # [[2 * np.pi / 4, -np.pi / 4, np.pi / 2], [0, 0, TRANS]],
            # [[3 * np.pi / 4, -np.pi / 4, np.pi / 2], [0, 0, TRANS]],
...
if __name__=='__main__':
    ...
    points=np.load('D:\Computer_vision\Dataset/3D_help_2D/ModelNet40-LS\pointclouds/airplane/airplane_0002.npy')
    points=np.random.permutation(points)[:2048,:3]
